"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getBatchIdAtIndex = exports.decodeGetBatchIdAtIndexResult = exports.encodeGetBatchIdAtIndex = exports.encodeGetBatchIdAtIndexParams = exports.isGetBatchIdAtIndexSupported = exports.FN_SELECTOR = void 0;
const read_contract_js_1 = require("../../../../../transaction/read-contract.js");
const encodeAbiParameters_js_1 = require("../../../../../utils/abi/encodeAbiParameters.js");
const viem_1 = require("viem");
const detectExtension_js_1 = require("../../../../../utils/bytecode/detectExtension.js");
exports.FN_SELECTOR = "0x2419f51b";
const FN_INPUTS = [
    {
        type: "uint256",
        name: "_index",
    },
];
const FN_OUTPUTS = [
    {
        type: "uint256",
    },
];
/**
 * Checks if the `getBatchIdAtIndex` method is supported by the given contract.
 * @param contract The ThirdwebContract.
 * @returns A promise that resolves to a boolean indicating if the `getBatchIdAtIndex` method is supported.
 * @extension ERC721
 * @example
 * ```ts
 * import { isGetBatchIdAtIndexSupported } from "thirdweb/extensions/erc721";
 *
 * const supported = await isGetBatchIdAtIndexSupported(contract);
 * ```
 */
async function isGetBatchIdAtIndexSupported(contract) {
    return (0, detectExtension_js_1.detectMethod)({
        contract,
        method: [exports.FN_SELECTOR, FN_INPUTS, FN_OUTPUTS],
    });
}
exports.isGetBatchIdAtIndexSupported = isGetBatchIdAtIndexSupported;
/**
 * Encodes the parameters for the "getBatchIdAtIndex" function.
 * @param options - The options for the getBatchIdAtIndex function.
 * @returns The encoded ABI parameters.
 * @extension ERC721
 * @example
 * ```ts
 * import { encodeGetBatchIdAtIndexParams } "thirdweb/extensions/erc721";
 * const result = encodeGetBatchIdAtIndexParams({
 *  index: ...,
 * });
 * ```
 */
function encodeGetBatchIdAtIndexParams(options) {
    return (0, encodeAbiParameters_js_1.encodeAbiParameters)(FN_INPUTS, [options.index]);
}
exports.encodeGetBatchIdAtIndexParams = encodeGetBatchIdAtIndexParams;
/**
 * Encodes the "getBatchIdAtIndex" function into a Hex string with its parameters.
 * @param options - The options for the getBatchIdAtIndex function.
 * @returns The encoded hexadecimal string.
 * @extension ERC721
 * @example
 * ```ts
 * import { encodeGetBatchIdAtIndex } "thirdweb/extensions/erc721";
 * const result = encodeGetBatchIdAtIndex({
 *  index: ...,
 * });
 * ```
 */
function encodeGetBatchIdAtIndex(options) {
    // we do a "manual" concat here to avoid the overhead of the "concatHex" function
    // we can do this because we know the specific formats of the values
    return (exports.FN_SELECTOR +
        encodeGetBatchIdAtIndexParams(options).slice(2));
}
exports.encodeGetBatchIdAtIndex = encodeGetBatchIdAtIndex;
/**
 * Decodes the result of the getBatchIdAtIndex function call.
 * @param result - The hexadecimal result to decode.
 * @returns The decoded result as per the FN_OUTPUTS definition.
 * @extension ERC721
 * @example
 * ```ts
 * import { decodeGetBatchIdAtIndexResult } from "thirdweb/extensions/erc721";
 * const result = decodeGetBatchIdAtIndexResult("...");
 * ```
 */
function decodeGetBatchIdAtIndexResult(result) {
    return (0, viem_1.decodeAbiParameters)(FN_OUTPUTS, result)[0];
}
exports.decodeGetBatchIdAtIndexResult = decodeGetBatchIdAtIndexResult;
/**
 * Calls the "getBatchIdAtIndex" function on the contract.
 * @param options - The options for the getBatchIdAtIndex function.
 * @returns The parsed result of the function call.
 * @extension ERC721
 * @example
 * ```ts
 * import { getBatchIdAtIndex } from "thirdweb/extensions/erc721";
 *
 * const result = await getBatchIdAtIndex({
 *  contract,
 *  index: ...,
 * });
 *
 * ```
 */
async function getBatchIdAtIndex(options) {
    return (0, read_contract_js_1.readContract)({
        contract: options.contract,
        method: [exports.FN_SELECTOR, FN_INPUTS, FN_OUTPUTS],
        params: [options.index],
    });
}
exports.getBatchIdAtIndex = getBatchIdAtIndex;
//# sourceMappingURL=getBatchIdAtIndex.js.map