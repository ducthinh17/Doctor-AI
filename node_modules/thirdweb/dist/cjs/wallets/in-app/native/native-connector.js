...
class InAppNativeConnector {
    ...
    async getUser() {
        ...
        // Logged out
        return { status: type_js_1.UserWalletStatus.LOGGED_OUT };
    }
    getAccount() {
        return (0, retrieval_js_1.getExistingUserAccount)({ client: this.options.client });
    }
    preAuthenticate(params) {
        const strategy = params.strategy;
        switch (strategy) {
            case "email": {
                return (0, auth_js_1.sendVerificationEmail)({
                    email: params.email,
                    client: this.options.client,
                });
            }
            case "phone":
                return (0, auth_js_1.sendVerificationSms)({
        ...
        switch (strategy) {
            ...
            case "apple": {
                if (!params.redirectUrl) {
                    throw new Error("redirectUrl deeplink is required for oauth login (ex: myApp://) - You also need add this deeplink in your allowed `Redirect URIs` under your API Key in your thirdweb dashboard: https://thirdweb.com/dashboard/settings/api-keys");
                }
    ...
    async validateEmailOTP(options) {
        try {
            const { storedToken } = await (0, auth_js_1.validateEmailOTP)({
                email: options.email,
                client: this.options.client,
                otp: options.otp,
    ...
    async deleteActiveAccount() {
        return (0, auth_js_1.deleteActiveAccount)({ client: this.options.client });
    }
    async socialLogin(oauthOption) {
        try {
            const { storedToken } = await (0, auth_js_1.socialLogin)(oauthOption, this.options.client);
            const account = await this.getAccount();
            return {
                user: {
                    status: type_js_1.UserWalletStatus.LOGGED_IN_WALLET_INITIALIZED,
            ...
            if (error instanceof Error) {
                throw new Error(`Error signing in with ${oauthOption.provider}: ${error.message}`);
            }
            throw new Error(`An unknown error occurred signing in with ${oauthOption.provider}`);
        }
    }
    async customJwt(authOptions) {
        try {
            const { storedToken } = await (0, auth_js_1.customJwt)(authOptions, this.options.client);
            const account = await this.getAccount();
            return {
                user: {
                    status: type_js_1.UserWalletStatus.LOGGED_IN_WALLET_INITIALIZED,
    ...
    async authEndpoint(authOptions) {
        try {
            const { storedToken } = await (0, auth_js_1.authEndpoint)(authOptions, this.options.client);
            const account = await this.getAccount();
            return {
                user: {
                    status: type_js_1.UserWalletStatus.LOGGED_IN_WALLET_INITIALIZED,
