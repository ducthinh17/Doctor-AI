...
import { AUTH_TOKEN_LOCAL_STORAGE_NAME, DEVICE_SHARE_LOCAL_STORAGE_NAME, WALLET_USER_DETAILS_LOCAL_STORAGE_NAME, WALLET_USER_ID_LOCAL_STORAGE_NAME, } from "../../../core/constants/settings.js";
...
export async function saveConnectedEmail(email) {
...
export async function getConnectedAuthStrategy() {
    return (await getItemFromAsyncStorage(CONNECTED_AUTH_STRATEGY_LOCAL_STORAGE_NAME));
}
export async function saveConnectedAuthStrategy(authStrategy) {
...
export async function isDeviceSharePresentForUser(clientId, walletUserId) {
    ...
    const storedDeviceShare = await getItemFromAsyncStorage(DEVICE_SHARE_LOCAL_STORAGE_NAME(clientId, walletUserId));
...
export async function getWalletUserDetails(clientId) {
    const result = await getItemFromAsyncStorage(WALLET_USER_DETAILS_LOCAL_STORAGE_NAME(clientId));
    if (!result) {
        return undefined;
    }
    try {
