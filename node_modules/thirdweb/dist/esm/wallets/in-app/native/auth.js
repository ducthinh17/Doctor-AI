...
import { AuthProvider, RecoveryShareManagement, } from "../core/authentication/type.js";
import { deleteAccount, fetchUserDetails, generateAuthTokenFromCognitoEmailOtp, getSessionHeaders, verifyClientId, } from "./helpers/api/fetchers.js";
import { cognitoEmailSignIn, cognitoEmailSignUp, cognitoPhoneSignIn, cognitoPhoneSignUp, } from "./helpers/auth/cognitoAuth.js";
import { postAuth, postAuthUserManaged, preAuth, } from "./helpers/auth/middleware.js";
import { DOMAIN_URL_2023, ROUTE_AUTH_ENDPOINT_CALLBACK, ROUTE_AUTH_JWT_CALLBACK, ROUTE_HEADLESS_OAUTH_LOGIN, } from "./helpers/constants.js";
...
export async function sendVerificationEmail(options) {
    ...
    try {
        ...
        throw new Error(createErrorMessage("Malformed response from the send email OTP API", e));
    }
    let cognitoUser;
    try {
        cognitoUser = await cognitoEmailSignIn(options.email, options.client.clientId);
    }
    catch (e) {
    ...
    return {
        ...
        isNewDevice: !(await isDeviceSharePresentForUser(options.client.clientId, result.walletUserId ?? "")),
...
export async function sendVerificationSms(options) {
    ...
    let cognitoUser;
    try {
        cognitoUser = await cognitoPhoneSignIn(options.phoneNumber, options.client.clientId);
    }
    catch (e) {
...
export async function validateEmailOTP(options) {
    try {
        ...
        throw new Error(createErrorMessage("Malformed response validating the OTP", e));
    }
    ...
    try {
        ...
        cognitoUser = await Auth.sendCustomChallengeAnswer(cognitoUser, options.otp);
        // It we get here, the answer was sent successfully,
        // but it might have been wrong (1st or 2nd time)
        // So we should test if the user is authenticated now
        const session = await Auth.currentSession();
        verifiedTokenResponse = await generateAuthTokenFromCognitoEmailOtp(session, options.client.clientId);
    }
    catch (e) {
    ...
    try {
        ...
        throw new Error(createErrorMessage("Malformed response from the verify one time password", e));
    }
}
export async function socialLogin(oauthOptions, client) {
    const encodedProvider = encodeURIComponent(oauthOptions.provider);
    const headlessLoginLinkWithParams = `${ROUTE_HEADLESS_OAUTH_LOGIN}?authProvider=${encodedProvider}&baseUrl=${encodeURIComponent(DOMAIN_URL_2023)}&platform=${encodeURIComponent("mobile")}`;
