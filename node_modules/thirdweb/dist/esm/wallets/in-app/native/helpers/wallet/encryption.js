...
import { base64ToUint8Array, uint8ArrayToBase64, } from "../../../../../utils/uint8-array.js";
const ENCRYPTION_SEPARATOR = ":";
const DEPRECATED_KEY_ITERATION_COUNT = 5000000;
const CURRENT_KEY_ITERATION_COUNT = 650000;
const KEY_LENGTH = 256;
export async function getEncryptionKey(pwd, salt, iterationCounts) {
    // @ts-ignore - default import buils but ts doesn't like it
    const key = QuickCrypto.pbkdf2Sync(pwd, salt.buffer, iterationCounts, KEY_LENGTH, "SHA-256");
...
export async function encryptShareWeb(share, pwd) {
    ...
    // iv is a hex string
    const ivBase64 = uint8ArrayToBase64(hexToUint8Array(`0x${encryptedValue.iv}`));
    const returnValue = `${uint8ArrayToBase64(cipherTextWithTag)}${ENCRYPTION_SEPARATOR}${ivBase64}${ENCRYPTION_SEPARATOR}${uint8ArrayToBase64(salt)}${ENCRYPTION_SEPARATOR}${iterationCount}`;
    return returnValue;
}
export async function decryptShareWeb(encryptedShareDetails, pwd) {
    ...
    base64ToUint8Array(saltBase64), iterationCount);
    ...
    // @ts-ignore - default import buils but ts doesn't like it
    const normalizedShare = await AesGcmCrypto.decrypt(originalBase64CipherText, key, ivBufferHex.slice(2), // lib expects hex with no
