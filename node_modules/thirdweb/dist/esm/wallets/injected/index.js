 ...
 */
export function isInjectedWallet(wallet) {
 ...
 * @internal
 */
export async function connectInjectedWallet(id, options, emitter) {
 ...
 * @internal
 */
export async function autoConnectInjectedWallet(id, emitter, chain) {
    const provider = getInjectedProvider(id);
    // connected accounts
    const addresses = await provider.request({
        method: "eth_accounts",
...
function createAccount(provider, address) {
    const account = {
        ...
        async signTypedData(typedData) {
            ...
            const stringifiedData = stringify({ domain: domain ?? {}, message, primaryType, types }, (_, value) => (isHex(value) ? value.toLowerCase() : value));
            return await provider.request({
