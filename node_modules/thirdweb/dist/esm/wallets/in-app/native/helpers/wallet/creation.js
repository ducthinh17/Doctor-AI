...
import { hexToUint8Array, stringToHex, toHex, uint8ArrayToHex, } from "../../../../../utils/encoding/hex.js";
import { storeUserShares } from "../api/fetchers.js";
import { logoutUser } from "../auth/logout.js";
import { AUTH_SHARE_INDEX, DEVICE_SHARE_INDEX, DEVICE_SHARE_MISSING_MESSAGE, RECOVERY_SHARE_INDEX, } from "../constants.js";
...
export async function setUpNewUserWallet(recoveryCode, client) {
    try {
 ...
 */
export async function storeShares({ client, walletAddress, authShare, deviceShare, recoveryShares, }) {
    ...
    if (recoveryShares?.length) {
        maybeEncryptedRecoveryShares = await Promise.all(recoveryShares.map(async (recoveryShare) => {
            return {
                share: await encryptShareWeb(recoveryShare.share, recoveryShare.recoveryCode),
    ...
    try {
        ...
        throw new Error(createErrorMessage("Malformed response from the ews store user share API", e));
    }
}
